...
((select id from "COACH" where user_id = (select id from "USER" where email = 'lee2000@hexschooltest.io')),(select id from "SKILL" where name = '重訓' )),
((select id from "COACH" where user_id = (select id from "USER" where email = 'muscle@hexschooltest.io')),(select id from "SKILL" where name = '重訓' )),
((select id from "COACH" where user_id = (select id from "USER" where email = 'starplatinum@hexschooltest.io')),(select id from "SKILL" where name = '重訓' )),
...
select id, user_id, course_id, booking_at, status, join_at, leave_at, cancelled_at, cancellation_reason, created_at 
from "COURSE_BOOKING"
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io');
...
select user_id, SUM(purchased_credits) as credits_Total, SUM(price_paid) as price_total
...
select "CREDIT_PURCHASE".user_id, "CREDIT_PURCHASE".credits_qty, "COURSE_BOOKING".booking_qty,
"CREDIT_PURCHASE".credits_qty-"COURSE_BOOKING".booking_qty as remaining_credit from 
(select user_id, sum(purchased_credits) as credits_qty
from "CREDIT_PURCHASE"
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
group by user_id) as "CREDIT_PURCHASE" 
inner join
(select user_id, count(*) as booking_qty
from "COURSE_BOOKING"
where user_id = (select id from "USER" where email = 'wXlTq@hexschooltest.io')
and status = '上課中'
group by user_id) as "COURSE_BOOKING"
on "CREDIT_PURCHASE".user_id = "CREDIT_PURCHASE".user_id;
...
select "USER".id, "USER"."name", "SKILL".name as Expertise_name, "COACH".experience_years 
from "SKILL"
...
select "SKILL".name as Expertise_name,count(*) as coach_total
...
select name, sum(credit_amount) as credit_total,sum(price) as price_total 
from "CREDIT_PACKAGE"
where TO_CHAR(created_at::date, 'yyyy/mm') = '2024/11' 
group by name;
...
select count(distinct user_id) as user_qty
from "COURSE_BOOKING"
